...
// avatarFilenameRe matches avatar filenames generated by the backend ({userID}_{8hex}.jpg).
// Used to validate old avatar paths before deletion so a tampered avatar_url can't
// be used to delete arbitrary files in the avatars directory.
var avatarFilenameRe = regexp.MustCompile(`^\d+_[0-9a-f]{8}\.jpg$`)
...
func (h *Handler) UploadAvatar(c *gin.Context) {
	...
	if oldUser, _ := h.authService.GetUserByID(c.Request.Context(), userID); oldUser != nil && oldUser.AvatarURL != "" {
		name := filepath.Base(oldUser.AvatarURL)
		if avatarFilenameRe.MatchString(name) {
			os.Remove(filepath.Join("uploads/avatars", name))
		}
	}
